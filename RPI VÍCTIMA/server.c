...
void my_memcpy(byte *dst, byte *src, int n) {
  int i;
  for (i=0;i<n;i++) {
    dst[i] = src[i];
  }
...
/*
#define sub_bytes_cfg(a,n,i)			\
  for(i=0;i<n;++i) a[i]=lookup_sbox[a[i]]
#define sub_bytes_inv(a,n,i)			\
  for(i=0;i<n;++i) a[i]=lookup_sbox_inv[a[i]]
*/
...
void key_schedule_core(byte *a, int i) { 
  ...
  a[2]=a[3];
  a[3]=temp;
  sub_bytes_cfg(a,4);       // Apply Rijndael's S-box on all four individual bytes in the output word
...
void expand_key_cfg(byte *key, byte *keys) {
  ...
  my_memcpy(keys,key,16);      // The first 16 bytes of the expanded key are simply the encryption key
  
  while (bytes<176) {       // Until we have 176 bytes of expanded key, we do the following:
    my_memcpy(t,keys+bytes-4,4);          // We assign the value of the previous four bytes in the expanded key to t
    ...
    my_memcpy(keys+bytes,t,4);            // This becomes the next 4 bytes in the expanded key
    ...
    for (j=0;j<3;j++) {
      my_memcpy(t,keys+bytes-4,4);          // We assign the value of the previous 4 bytes in the expanded key to t
...
void shift_rows_cfg(byte *state) {
  int i;
  byte temp[16];
  my_memcpy(temp,state,16);
  for (i=0;i<16;i++)
...
void mix_cols_cfg (byte *state) {
  mix_col_cfg(state);
  mix_col_cfg(state+4);
  mix_col_cfg(state+8);
  mix_col_cfg(state+12);
}
...
void EncryptAES_cfg(byte *msg, byte *key, byte *c) {
  int i; // To count the rounds
  
  // Key expansion
  byte keys[176];
  expand_key_cfg(key,keys);
  
  // First Round
  my_memcpy(c, msg, 16);
  xor_round_key_cfg(c,keys,0);
  ...
  for(i=0; i<9; i++) {
    ...
    shift_rows_cfg(c);
    mix_cols_cfg(c);
    xor_round_key_cfg(c, keys, i+1);
  }

  // Final Round
  sub_bytes_cfg(c,16);
  shift_rows_cfg(c);
  xor_round_key_cfg(c, keys, 10);
}
...
void DecryptAES(byte *c, byte *key, byte *m) {
  ...
  my_memcpy(m,c,16);
  xor_round_key_cfg(m,keys,10);
  shift_rows_inv(m);
  sub_bytes_inv(m, 16);
  
  // Reverse the middle rounds
  for (i=0; i<9; i++) {
    xor_round_key_cfg(m,keys,9-i);
    mix_cols_inv(m);
    shift_rows_inv(m);
    sub_bytes_inv(m, 16);
  }
...
void
intptr2ascii (char* dst, byte* src, byte len)
{
  byte i, j;

  for (i = 0; i < len; i++) {
    for (j = 0; j < 8; j++) {
      *(dst++) = bit(src[i], 7-j) ? '1' : '0';
    }
  }
  *dst = '\0';
}
...
void hexstr_to_char(const char* hexstr,byte* msg)
{
   ...
   // unsigned char* chrs = (unsigned char*)malloc((final_len+1) * sizeof(*chrs));
    for (size_t i=0, j=0; j<final_len; i+=2, j++)
        msg[j] = (hexstr[i] % 32 + 9) % 25 * 16 + (hexstr[i+1] % 32 + 9) % 25;
    msg[final_len] = '\0';
    return;
}
...
//intptr2ascii (buffer1, key, 16);
                               
    //  for (i = 1, len++; i < len; i++) {
    
      //CharToByte( argv[1],msg,sizeof(msg));
      
      hexstr_to_char(buffer,msg);
     
      
      digitalWrite (0, 1);//Este va para la interrupcion gpio17
      
     delay(10);
      EncryptAES_cfg (msg, key, encrypted);
    // delay(1000);
    digitalWrite(0,0); 
    digitalWrite(1,1);
   delay(10); 
    digitalWrite(1,0);//Este para la 18 //Estan los pines conectados uno a uno por cable
     // intptr2ascii (buffer1, encrypted, 16);
      
     // intptr2ascii (buffer1, msg, 16);
      

      
      Pretty(msg, sizeof(msg), "Input: 0x");
      Pretty(key, sizeof(key), "Key: 0x");
      Pretty(encrypted, sizeof(encrypted), "Output: 0x");
